(function($){
    ...
    //   	});
				...
				request.then(function(data){
					if(data == "1"){
						...
						if(cf.find('input').val() == ""){
							var text = "All required fields must be filled!";
						} else {
							var text = "Заявка на замер успешно отправлена";
						}
		...
		if ($('#contact_form2').length){
			...
			var cf2 = $('#contact_form2');
			cf2.prepend('<div class="message_container2"></div>');

			cf2.on("submit",function(event){

				var self = $(this),text;

				var request = $.ajax({
					url:"backend/contact_form2.php",
					type : "post",
					data : cf2.serialize(),
					success: function() {
						var text = "Ваша заявка успешно отправлена!";
				...
				request.then(function(data){
					...
					//
					// 	cf.find('input:not([type="submit"])').val('');
					//
					// 	$('.message_container2').html('<div class="alert_box success"><i class="icon-smile"></i><p>'+text+'</p></div>')
					// 		.delay(150)
					// 		.slideDown(300)
		...
		// Calculate the cost [contact_form3]
		if ($('#contact_form3').length){
			var cf3 = $('#contact_form3');
			cf3.append('<div class="message_container3"></div>');

			cf3.on("submit",function(event){

				var self = $(this),text;

				var request = $.ajax({
					url:"backend/contact_form3.php",
					type : "post",
					data : cf3.serialize(),
					success: function() {
						var formHeight = cf3.height();
						var cform3 = $("#contact_form_three");
						cf3.fadeOut(1000);
						cform3.css("height", formHeight + "px");
						cform3.append(
							"<div class='row'>" +
								"<div class='col-md-12 cform3-appended'>" +
				...
				request.then(function(data){
					...
					if(data == "1"){
						...
						cf3.find('input:not([type="submit"]),textarea').val('');
